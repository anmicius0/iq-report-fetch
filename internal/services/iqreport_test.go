...
func TestGenerateLatestPolicyReport_Integration(t *testing.T) {
	...
	filename := "report.csv"
	outputPath, err := svc.GenerateLatestPolicyReport(rCtx(t), filename)
	if err != nil {
...
func TestGenerateLatestPolicyReport_GetApplicationsError(t *testing.T) {
	// Server that returns 500 for applications
	...
	iqClient, _ := client.NewClient(baseURL, "u", "p", testLogger())
	cfg := &config.Config{OutputDir: t.TempDir()}
	svc := NewIQReportService(cfg, iqClient, testLogger())

	_, err := svc.GenerateLatestPolicyReport(context.Background(), "report.csv")
	if err == nil {
		t.Fatal("expected error, got nil")
	}
	if !strings.Contains(err.Error(), "get applications") {
		t.Errorf("expected 'get applications' error, got %v", err)
	}
}

func TestGenerateLatestPolicyReport_NoApplicationsFound(t *testing.T) {
	// Server that returns empty applications list
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if strings.Contains(r.URL.Path, "applications") {
			w.Header().Set("Content-Type", "application/json")
	...
	if !strings.Contains(err.Error(), "no applications found") {
		t.Errorf("expected 'no applications found' error, got %v", err)
	}
}
